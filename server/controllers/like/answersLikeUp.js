...
module.exports = async (req, res) => {
  ...
  try {
    const check_403_Sql = `SELECT * FROM ANSWER WHERE ANSWER_ID = ${answerId} AND USER_ID = ${accessTokenData.USER_ID};`;
    ...
    pool.query(
      check_403_Sql + check_404_Sql /*+ check_409_Sql*/,
      (err, result) => {
        if (err) {
          console.error(err);
