...
const start = (req, res) => {
  ...
  if ( req.body.pull_request.base.repo.id != process_env.GITHUB_REPO_ID ) return res.status(200).send('njx NOT OK');
  if ( req.body.pull_request.merged == false ) return res.status(200).send('njx NOT OK');
  ...
  console.log(`cwd = ${CWD}`)
  fs.writeFileSync(`${CWD}/.repo`, 'git@github.com:'+req.body.pull_request.base.repo.full_name);
  ...
  switch (req.body.pull_request.base.ref) {
    ...
    case 'dev-back':
      ...
      fs.writeFileSync(`${CWD}/server/dev-back`,  process_env.GITHUB_DEPLOY_SERVER);      
      break;
  }
